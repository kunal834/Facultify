...
// Standalone invite-acceptance page — NOT inside the student layout.
export default function StudentInvitePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // pre-fetched by an email client before the student clicked the link.
      if (params.get("error")) {
        setError("This invite link has expired or has already been used. Please ask your teacher to send a new invite.");
        return;
      }
      ...
      if (!hash.includes("access_token")) {
        setError("This invite link is invalid. Please ask your teacher to send a new invite.");
        return;
      }
      ...
      if (sessionError) {
        setError("This invite link has expired or is invalid. Please ask your teacher to send a new invite.");
        return;
      }
      ...
      try {
        ...
        router.replace(json.destination ?? "/student");   // finalize returns /auth/set-password?next=/student on fresh invites
      } catch {
